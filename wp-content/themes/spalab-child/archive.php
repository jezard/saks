<?php get_header();?>
<?php $page_layout  = dttheme_option( 'specialty', 'tag-archives-layout' );
...
<?php if(!$is_blog_page): ?>
  <h2>All Services. Consider using the main menu or search to find what you're looking for...</h2>
<?php endif; ?>
<?php	if( have_posts() ):
			$i = 1;
			while( have_posts() ):
      ...
      $link = isset( $item_meta['link']) ? $item_meta['link'] : NULL; 
                ...
                <div class="<?php echo esc_attr( $temp_class ); ?>">
                    ...
                    <article id="post-<?php the_ID(); ?>" <?php post_class('blog-entry'); ?>>
                    	<!-- .blog-entry-inner -->
                        <div class="blog-entry-inner" style="<?php echo ($is_blog_page ? '' : 'min-height:0;padding:0'); ?>">
                          <?php if($is_blog_page): ?>
                        	<!-- .entry-meta -->
